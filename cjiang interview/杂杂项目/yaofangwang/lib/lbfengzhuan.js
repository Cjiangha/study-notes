...
function carouselimg(opt) {
    ...
    */
    ...
    bigpic.style = box.style = `width:${dafaultData.iw}%;height:${dafaultData.ih}px;`;
    //设置按钮外面盒子的宽高，让它定死在左右两边
    // posibtn.style.width = light.style.width = `${dafaultData.iw}px`;
    light.style.width = `${dafaultData.iw}%`;

    //2、渲染数据
    var pj = data.map(function (item) {
        return `<li style ="width:${dafaultData.iw}%;height:${dafaultData.ih}px;"><img src="${item}" alt=""></li>`;
    }).join('');
    ...
    function next() {
        //旧图挪走
        startMove(imglist[num], { 'left': -iw + 'px ' });
        //新图进场
        // num++;
        num = ++num > imglist.length - 1 ? 0 : num;//当数量>4的时候，就变为0；否则执行继续自增
