...
function magniglass(opt) {
    ...
    // console.log(smaller.getElementsByTagName('li')[0], biger.children[0]);
    smaller.innerHTML = html;//渲染小图
    smaller.lastElementChild.style.marginRight = 0;

    //设置小图ul宽度
    smaller.style.width = 98 * (smaller.children.length + 1) + 'px';
    ...
    // 默认speed为1，
    var iW = (smaller.children[0].offsetWidth + 25) * defaultopt.speed;//一个li的宽度
    // console.log(smaller.children[0].offsetWidth + 25);
    ...
    function move(speed) {//小图的运动
        ...
        // offsetWidth为这个盒子的宽度-
        if (left <= box.offsetWidth - smaller.offsetWidth + 25) {//临界值,25为最外边margin值，60
            left = box.offsetWidth - smaller.offsetWidth + 25;
