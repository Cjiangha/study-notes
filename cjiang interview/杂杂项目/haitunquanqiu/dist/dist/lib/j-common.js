...
function changeTime(time, type) {
    ...
    var month = toDb(time.getMonth() + 1) - 0;
...
function formatSeconds(value) {
    ...
    if (secondTime > 60) {//如果秒数大于60，将秒数转换成整数
        ...
        if (minuteTime > 60) {
            ...
            //获取小时后取佘的分，获取分钟取佘60的分
            minuteTime = parseInt(minuteTime % 60);
        }
    }
    var result = "" + toDb(parseInt(secondTime)) + "秒";
    if (minuteTime > 0) {
        result = "" + toDb(parseInt(minuteTime)) + "分" + result;
    }
    else if (minuteTime == 0) {
        result = "" + toDb(0) + "分" + result;
    }
    if (hourTime > 0) {
        result = "" + toDb(parseInt(hourTime)) + "小时" + result;
    }
    else if (hourTime == 0) {
        result = "" + toDb(0) + "小时" + result;
    }
...
var checkReg = {
    ...
    email: function (str) {
        var reg = /^[\w&%$#!\-]+@[\w&%$#!\-]+\.[a-zA-Z]+$/;
        return reg.test(str);//返回true或者false
    },
    // 验证电话
    tel: function (str) {
        var reg = /^1[3-9]\d{9}$/;
...
function checkInput(ele, reg, inf, meg) {
    /*
      ...
      参数四：meg 提示信息不同,对象
            比如： 
        checkInput(ainputs[1], 'tel', infs[1], {
                success: '身份证输入正确',
                failure: '身份证输入错误',
                null: '身份证不能为空'
        });
    */
    ...
    ele.onblur = function () {
        ...
        if (val) {
            //2.正则验证
            var res = checkReg[reg](val);//方法二：利用对象属
            if (res) {
                //符合规则
                inf.innerHTML = meg.success;//提示信息渲染到提示信息节点的节点上，
                inf.style.color = '#58bc58';//符合时提示颜色
                inf.parentNode.style = 'display:none';
                ele.istrue = true;//正确
            } else {
                //不符合规则
                inf.parentNode.style = 'display:block';
                inf.innerHTML = meg.failure;//提示信息渲染到提示信息节点的节点上
                inf.style.color = 'red';//不符合时提示颜色
                ele.istrue = false;//错误
            }
        } else {
            inf.parentNode.style = 'display:block';
            inf.innerHTML = meg.null;//提示信息渲染到提示信息节点的节点上
            inf.style.color = 'red';//不符合时的提示颜色
            ele.istrue = false;//错误
        }
    }
...
function checkInput2(ele, reg, inf, meg) {
    /*
      参数一：ele 要正则验证的表单
      参数二：reg 正则不同
      参数三：inf 提示信息节点不同
    ...
    xhr.onreadystatechange = () => {
        if (xhr.readyState == 4) {
            if (xhr.status == 200 || xhr.status == 304) {
                //成功了
                let data = xhr.responseText;//响应的Text值
...
//设置cookie
// setCookie('username',后台成功返回的值,10)
