		...
		for(let j=0; j< signPan[i].length; j++){
			...
			switch (signPan[i][j]){
				...
				case 4://静态数字
					...
					cxt.font="700 "+fontSize+"px sans-serif";
					key = String.fromCharCode(i+97) + String.fromCharCode(j+97);
					cxt.fillText(numSignRecord[key], x, y+minWH/2);
					break;
				case 5://字母
					cxt.fillStyle = signColor_;
					key = String.fromCharCode(i+97) + String.fromCharCode(j+97);
					...
					if(pan[i][j] == 0){
						...
						cxt.fillText(zimu, x, y+minWH*1/2);
					}else{
						fontSize = minWH;
						cxt.font="500 "+fontSize+"px sans-serif";
				...
				case 6://三角形
					...
					let h1 = st_/Math.sin((Math.PI/3))
					let h2 = st_*2*Math.sin(Math.PI/3)-h1;

					cxt.strokeStyle=signColor_;
					cxt.lineCap="round";
					cxt.moveTo(x, y-h1);
					cxt.lineTo(x-st_,y+h2);
					cxt.stroke();
					cxt.moveTo(x, y-h1);
					cxt.lineTo(x+st_,y+h2);
					cxt.stroke();

					cxt.moveTo(x-st_,y+h2);
					cxt.lineTo(x+st_,y+h2);
					cxt.stroke();
...
function regretDrawStep(){
	if(move_count == 0){
		alert("已经不能再悔了");
		return false;
	}
	
	if(drawGoCross == undefined || drawGoCross == ''  ){
		console.log('没有动态步骤可悔棋');
		return false;
	}
	//将悔的动态棋子信息拿到
	let lastGoCross_ = move_record[move_record.length-1];
	let r_ = lastGoCross_[0], c_ = lastGoCross_[1], move_count_ = lastGoCross_[2];
	let color_ = move_count_%2==0 ? 'W' : 'B';
	if(move_count_ < 10){
		move_count_ = '0'+move_count_;
	}
	drawGoCross += color_+ String.fromCharCode(r_+97) + String.fromCharCode(c_+97) +
					'R' + move_count_+'000';
...
function setDynGoCrosses(dynGoCrosses){
	console.log("dynGoCrosses====>"+dynGoCrosses);
...
var kps = new Array(), kpscolor='#F9631F';
...
function drawKPS(cxt){
	...
	let fontsize = minW*7/9;
	cxt.font="400 "+fontsize+"px NotoSansHans, NotoSansHans-Medium";
	cxt.textAlign='center';
	for(let i=0; i<kps.length; i++){
		...
		switch (p){
			case 1 : 
				...
				break;
			case 2 : 
				x=minW*10,y=minW*3.8;
				break;
			case 3 : 
				x=minW*17,y=minW*3.8;
				break;
			case 4 : 
				x=minW*3,y=minW*16.8;
				break;
			case 5 : 
				x=minW*10,y=minW*16.8;
				break;
			case 6 : 
				x=minW*17,y=minW*16.8;
				break;
		}
		cxt.fillStyle=kpscolor;
		if(kp.color != null && kp.color != ''){
			cxt.fillStyle=kp.color;
		}
		// cxt.fillText(kpdesc, x, y);
		drawrowfont(kpdesc, x, y, cxt);
	}
}

function drawrowfont(kpdesc, x, y, cxt){
	let w_ = minW*5;
	var chr = kpdesc.split("");
		var temp = "";                
		var row = [];
		for(var a = 0; a < chr.length; a++){
				if( cxt.measureText(temp).width < w_ ){
						;
				}else{
						row.push(temp);
						temp = "";
		...
		for(var b = 0; b < row.length; b++){
			cxt.fillText(row[b],x,y+(b+1)*minW);
		}
}
