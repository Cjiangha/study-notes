...
export default function debounce(func, wait, options) {
  var timerId,
    ...
    // 缓存的上一个 arguments
    lastCallTime,
    // 缓存的上一个 执行 debounced 的时间
    lastInvokeTime = 0,
    // 缓存的上一个 执行 invokeFunc 的时间
    maxWait,
    maxing = false,
    leading = false,
    trailing = true,
  ...
  function formatArgs() {
    ...
    function isObject(value) {
      ...
      var type = typeof value; // 确保 value 是 truthy 的值
  ...
  function timeExpired() {
    ...
    var realwait = remainingWait(time); // 重新启动计时器

    timerId = startTimer(realwait); // 计算真正延迟触发的时间

    function remainingWait(time) {
      var timeSinceLastCall = time - lastCallTime,
        // debounced 上次触发到现在的经历的时间
        timeSinceLastInvoke = time - lastInvokeTime,
        // invokeFunc 上次触发到现在的经历的时间
  ...
  function leadingEdge(time) {
    ...
    timerId = startTimer(wait); // Invoke the leading edge.
