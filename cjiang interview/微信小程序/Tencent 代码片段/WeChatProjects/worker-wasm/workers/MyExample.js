...
if (ENVIRONMENT_IS_NODE) {
    ...
    if (ENVIRONMENT_IS_WORKER) {} else if (document.currentScript) { scriptDirectory = document.currentScript.src }
...
function addFunctionWasm(func, sig) {
    var table = wasmTable;
    if (!functionsInTableMap) { functionsInTableMap = new WeakMap; for (var i = 0; i < table.length; i++) { var item = table.get(i); if (item) { functionsInTableMap.set(item, i) } } }
    if (functionsInTableMap.has(func)) { return functionsInTableMap.get(func) }
    for (var i = 0; i < table.length; i++) { assert(table.get(i) != func, "function in Table but not functionsInTableMap") }
    var ret;
    if (freeTableIndexes.length) { ret = freeTableIndexes.pop() } else { ret = table.length; try { table.grow(1) } catch (err) { if (!(err instanceof RangeError)) { throw err } throw "Unable to grow wasm table. Set ALLOW_TABLE_GROWTH." } }
    try { table.set(ret, func) } catch (err) {
...
var wasmMemory;
var wasmTable = new WebAssembly.Table({ "initial": 1577, "maximum": 1577 + 0, "element": "anyfunc" });
var ABORT = false;
var EXITSTATUS = 0;
...
var STACK_BASE = 5441776,
    STACK_MAX = 198896,
    DYNAMIC_BASE = 5441776,
    DYNAMICTOP_PTR = 198736;
...
if (Module["wasmMemory"]) { wasmMemory = Module["wasmMemory"] } else { wasmMemory = new WebAssembly.Memory({ "initial": INITIAL_INITIAL_MEMORY / WASM_PAGE_SIZE, "maximum": INITIAL_INITIAL_MEMORY / WASM_PAGE_SIZE }) }
...
var wasmBinaryFile = "MyExample.wasm";
...
function createWasm() {
    ...
    function instantiateArrayBuffer(receiver) {
        return WXWebAssembly.instantiate('/index/MyExample.wasm', info).then(function(instance) {
                return instance;
    ...
    var wx_start_time = Date.now();
    instantiateAsync();
    var wx_duration = (Date.now() - wx_start_time);
    console.log(wx_duration, String(wx_duration) + 'ms');
    return {}
}
var tempDouble;
var tempI64;
...
function ___map_file(pathname, size) { setErrNo(63); return -1 }
var PATH = {
...
var FS = {
    ...
    createDefaultDevices: function() {
        ...
        if (!random_device) { random_device = function() { abort("no cryptographic support found for random_device. consider polyfilling it if you want to use something insecure like Math.random(), e.g. put this in a --pre-js: var crypto = { getRandomValues: function(array) { for (var i = 0; i < array.length; i++) array[i] = (Math.random()*256)|0 } };") } }
...
function _emscripten_get_sbrk_ptr() { return 198736 }
...
var _use_lib = Module["_use_lib"] = createExportWrapper("use_lib");
var _cv_test = Module["_cv_test"] = createExportWrapper("cv_test");
...
noExitRuntime = true;
run();
var run_duration = (Date.now() - start_time);
console.log('run done', String(run_duration) + 'ms');
module.exports = {
    Module: Module
}